    ...
    case NodeType.Character:
      {
        newNodeElement.innerHTML += `
          ...
          <div class="panel" style="max-height: 0px; display: flex; gap: 0px;">
            <textarea style="min-width: 50%; max-width: 50%; padding: 0px;">${(node as Character).like}</textarea>
            <textarea style="min-width: 50%; max-width: 50%; padding: 0px;">${(node as Character).dislike}</textarea>
          </div>
        `;
      }
      break;
